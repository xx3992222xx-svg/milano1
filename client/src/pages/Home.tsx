...
const categoryProducts = [
  { name: "بوتاتي حبوب درجه اولى", price: "28000", sku: "ML-BOT-001", stock: 0, order: 1, discount: "0%", options: 4, images: 3, tone: "amber", glyph: "✦", hidden: false },
  { name: "بوتاتي حبوب فاخر", price: "32000", sku: "ML-BOT-002", stock: 6, order: 2, discount: "10%", options: 3, images: 4, tone: "blue", glyph: "✦", hidden: false },
  { name: "بوتاتي حبوب محمص", price: "24500", sku: "ML-BOT-003", stock: 14, order: 3, discount: "5%", options: 2, images: 2, tone: "purple", glyph: "✦", hidden: false },
  { name: "بوتاتي حبوب عضوي", price: "41000", sku: "ML-BOT-004", stock: 8, order: 4, discount: "0%", options: 5, images: 3, tone: "green", glyph: "✦", hidden: true },
];
...
function CategoriesPage({ locale, onOpenCategory }: { locale: Locale; onOpenCategory: () => void }) {
  const ar = locale === "ar"; const [query, setQuery] = useState(""); const [categories, setCategories] = useState(categoryData);
  const filtered = categories.filter(item => item.name.toLowerCase().includes(query.toLowerCase()));
  return <div className="categories-page"><div className="categories-toolbar"><div className="category-search"><Search size={16} /><input value={query} onChange={e => setQuery(e.target.value)} placeholder={ar ? "بحث عن قسم..." : "Search category..."} /></div><div className="category-toolbar-actions"><button className="sort-category-button"><MoreVertical size={15} />{ar ? "ترتيب الأقسام" : "Sort categories"}</button><button className="primary-button" onClick={() => toast.success(ar ? "تم فتح نموذج إضافة قسم" : "Add category form opened")}><Plus size={15} />{ar ? "إضافة قسم" : "Add category"} +</button></div></div><div className="category-list">{filtered.map((item, index) => <article className="category-row" key={item.name} onClick={index === 0 ? onOpenCategory : undefined}><div className={`category-watermark ${item.tone}`}>{item.glyph}</div><div className={`category-thumb ${item.tone}`}>{item.glyph}</div><div className="category-copy"><h2>{item.name}</h2><span>{item.count} {ar ? "منتج" : "products"}</span></div><div className="category-actions"><button className="category-action edit" onClick={e => { e.stopPropagation(); toast.info(ar ? "تم فتح تعديل القسم" : "Edit category opened"); }}><Pencil size={14} /></button><button className="category-action image" onClick={e => { e.stopPropagation(); toast.info(ar ? "تغيير صورة القسم" : "Change category image"); }}><CloudUpload size={14} /></button><button className="category-action delete" onClick={e => { e.stopPropagation(); setCategories(categories.filter(category => category.name !== item.name)); toast.success(ar ? "تم حذف القسم" : "Category deleted"); }}><Trash2 size={14} /></button></div></article>)}</div>{filtered.length === 0 && <div className="empty-category-state">{ar ? "لا توجد أقسام مطابقة" : "No matching categories"}</div>}</div>;
}
function CategoryProductsPage({ locale, onBack }: { locale: Locale; onBack: () => void }) {
  const ar = locale === "ar"; const [query, setQuery] = useState(""); const [products, setProducts] = useState(categoryProducts); const filtered = products.filter(item => `${item.name} ${item.sku}`.toLowerCase().includes(query.toLowerCase()));
  return <div className="category-products-page"><div className="category-products-header"><button className="back-category-button" onClick={onBack}><ChevronLeft size={16} />{ar ? "رجوع للأقسام" : "Back to categories"}</button><div className="category-products-title"><h1>{ar ? "بوتاتي حبوب درجه اولى" : "First-grade Botati grains"}</h1><span>{ar ? "9 منتج في هذا القسم" : "9 products in this category"}</span></div><div className="product-list-tools"><div className="product-list-search"><Search size={15} /><input value={query} onChange={e => setQuery(e.target.value)} placeholder={ar ? "ابحث عن اسم المنتج / رمز المنتج" : "Search product name / SKU"} /></div><button className="product-search-button"><Search size={14} />{ar ? "بحث" : "Search"}</button><button className="square-tool"><Settings size={15} /></button><button className="square-tool"><MoreVertical size={15} /></button></div></div><div className="category-product-grid">{filtered.map(product => <article className={`category-product-card ${product.hidden ? "product-hidden" : ""}`} key={product.sku}><div className={`product-visual ${product.tone}`}><span className="product-glyph">{product.glyph}</span><div className="product-card-actions"><button className="card-action delete" onClick={() => { setProducts(products.filter(item => item.sku !== product.sku)); toast.success(ar ? "تم حذف المنتج" : "Product deleted"); }}><Trash2 size={13} /></button><button className={`card-action visibility ${product.hidden ? "is-hidden" : ""}`} onClick={() => setProducts(products.map(item => item.sku === product.sku ? { ...item, hidden: !item.hidden } : item))}>{product.hidden ? <EyeOff size={13} /> : <Eye size={13} />}</button><button className="card-action view" onClick={() => toast.info(ar ? "تم فتح عرض المنتج" : "Product preview opened")}><ExternalLink size={13} /></button></div></div><div className="category-product-info"><h2>{product.name}</h2><strong className="product-price">{product.price} <small>{ar ? "ريال يمني" : "YER"}</small></strong><div className="product-meta-grid"><span>SKU <b>{product.sku}</b></span><span>{ar ? "المخزون" : "Stock"} <b className={categoryStockClass(product.stock)}>{product.stock}</b></span><span>{ar ? "الترتيب" : "Order"} <b>{product.order}</b></span><span>{ar ? "خصم" : "Discount"} <b>{product.discount}</b></span><span>{ar ? "خيارات" : "Options"} <b>{product.options}</b></span><span>{ar ? "صور" : "Images"} <b>{product.images}</b></span></div><button className="edit-product-full" onClick={() => toast.success(ar ? "تم فتح تعديل المنتج" : "Product editor opened")}><Pencil size={14} />{ar ? "تعديل المنتج" : "Edit product"}</button></div></article>)}</div></div>;
}
