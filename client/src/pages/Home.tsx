...
function PageHeading({ locale, view, onAdd }: { locale: Locale; view: View; onAdd: () => void }) {
  ...
  return <div className="page-heading"><div><h1 className="page-title">{t("welcome")}</h1></div><div className="heading-actions"><button className="primary-button" onClick={onAdd}><Plus size={15} />{t("addProduct")}</button></div></div>;
}
...
function ProductEditor({ locale }: { locale: Locale }) {
  ...
  const [productForm, setProductForm] = useState({ name: "", sku: "", cost: "0", stock: "0", price: "0", description: "" });
  const createProduct = trpc.products.create.useMutation({ onSuccess: () => toast.success(ar ? "تم حفظ المنتج وظهر في المتجر" : "Product saved and published to the store"), onError: () => toast.error(ar ? "تعذر حفظ المنتج، تحقق من البيانات" : "Could not save the product") });
...
function ReceiptHistoryPage({ locale, onBack }: { locale: Locale; onBack: () => void }) {
  const ar = locale === "ar"; const [query, setQuery] = useState(""); const [filter, setFilter] = useState("all");
  const [rows, setRows] = useState([
    { id: "ORD-869832", product: "بولي حبوب ابيض / مقاس 40", qty: 1, customer: "بسب", method: "نقدي / كاش", date: "2026/9/11 4:39 م", amount: "29,500", status: "مسترجع", tone: "pink" },
    { id: "ORD-658779", product: "قميص بولو قطن بياقة بيضاء S", qty: 1, customer: "ل س", method: "نقدي / كاش", date: "2026/9/11 4:31 م", amount: "4,500", status: "مسترجع", tone: "pink" },
    { id: "ORD-267249", product: "تشيرت اديداس اسود / M", qty: 2, customer: "ب ب", method: "نقدي / كاش", date: "2026/9/11 4:31 م", amount: "10,000", status: "مسترجع", tone: "pink" },
    { id: "POS-NHI87H0Q2", product: "بولي حبوب ابيض / مقاس 42", qty: 2, customer: "نزيـن صبي", method: "تحويل / بنكي", date: "2026/9/8 12:05 م", amount: "56,000", status: "مكتمل", tone: "green" },
    { id: "POS-L19W40ZRM", product: "شراب اديداس اسود / كبير", qty: 5, customer: "نزيـن صبي", method: "نقدي / كاش", date: "2026/9/7 11:35 ص", amount: "5,500", status: "مكتمل", tone: "green" },
    { id: "POS-EIF086CW4", product: "كاب ميلانو اسود / موحد", qty: 1, customer: "نزيـن صبي", method: "نقدي / كاش", date: "2026/9/7 01:34 م", amount: "5,500", status: "مكتمل", tone: "green" },
    { id: "POS-LEGH0XXX", product: "شنطة جينز رياضية لونين", qty: 1, customer: "نزيـن صبي", method: "نقدي / كاش", date: "2026/9/7 01:20 م", amount: "5,500", status: "مكتمل", tone: "green" },
    { id: "ORD-817263", product: "تيشرت ساتان اسود XXL", qty: 1, customer: "محمد ميلانو", method: "نقدي / كاش", date: "2026/9/9 04:06 م", amount: "7,500", status: "مسترجع", tone: "pink" },
  ]);
  const visible = rows.filter(row => (filter === "all" || row.tone === filter) && `${row.id} ${row.product} ${row.customer}`.includes(query));
  const total = rows.reduce((sum, row) => sum + Number(row.amount.replaceAll(",", "")), 0);
  const money = total.toLocaleString("en-US");
  const action = (label: string, id: string) => toast.success(ar ? `${label} ${id}` : `${label}: ${id}`);
  return <div className="receipt-page"><div className="receipt-head"><div><h1>{ar ? "سجل الإيصالات والتقارير" : "Receipt history and reports"}</h1><p>{ar ? "إدارة المبيعات والمرتجعات وطباعة التقارير" : "Manage sales, returns and printable reports"}</p></div><button className="pos-exit" onClick={onBack}><ChevronRight size={13} />{ar ? "نقطة البيع" : "Point of sale"}</button></div><div className="receipt-toolbar"><div className="receipt-stat"><span className="receipt-stat-icon green"><CircleDollarSign size={19} /></span><div><small>{ar ? "إجمالي مبيعات (هذا الشهر)" : "Monthly sales"}</small><strong>{money} {ar ? "ريال يمني" : "YER"}</strong></div></div><div className="receipt-stat"><span className="receipt-stat-icon blue"><Printer size={18} /></span><div><small>{ar ? "عدد الفواتير" : "Invoices"}</small><strong>{rows.length}</strong></div></div><div className="receipt-filters"><div className="receipt-dates"><button>2026/9/1</button><span>–</span><button>2026/9/30</button></div><div className="receipt-filter-tabs">{[["all", "الكل"], ["green", "مكتمل"], ["pink", "مسترجع"]].map(([key, label]) => <button className={filter === key ? "active" : ""} key={key} onClick={() => setFilter(key)}>{ar ? label : key}</button>)}</div><button className="receipt-report" onClick={() => toast.success(ar ? "تم تجهيز تقرير المبيعات للطباعة" : "Sales report ready to print")}><Printer size={13} />{ar ? "عرض تقرير المبيعات" : "View sales report"}</button></div></div><section className="panel receipt-table-card"><div className="receipt-table-top"><strong>{visible.length} {ar ? "فاتورة معروضة" : "invoices"}</strong><div className="receipt-search"><Search size={14} /><input value={query} onChange={e => setQuery(e.target.value)} placeholder={ar ? "بحث برقم الإيصال أو المنتج..." : "Search receipt or product..."} /></div></div><div className="receipt-table-wrap"><table className="receipt-table"><thead><tr><th>#</th><th>{ar ? "رقم الفاتورة" : "Receipt"}</th><th>{ar ? "اسم المنتج" : "Product"}</th><th>{ar ? "الكمية" : "Qty"}</th><th>{ar ? "اسم العميل" : "Customer"}</th><th>{ar ? "طريقة الدفع" : "Payment"}</th><th>{ar ? "التاريخ والوقت" : "Date / time"}</th><th>{ar ? "إجمالي الطلب" : "Total"}</th><th>{ar ? "الحالة" : "Status"}</th><th>{ar ? "إجراءات" : "Actions"}</th></tr></thead><tbody>{visible.map((row, index) => <tr key={row.id}><td>{index + 1}</td><td><b>{row.id}</b></td><td><strong>{row.product}</strong></td><td>{row.qty}</td><td>{row.customer}</td><td><span className="receipt-method">{row.method}</span></td><td dir="ltr">{row.date}</td><td><b>{row.amount} <small>{ar ? "ريال يمني" : "YER"}</small></b></td><td><span className={`pos-status ${row.tone}`}>{row.status}<i /></span></td><td><div className="receipt-actions"><button className="receipt-edit" onClick={() => action(ar ? "تم فتح تعديل" : "Edit opened", row.id)}><Pencil size={12} /></button><button className="receipt-print" onClick={() => action(ar ? "تم إرسال للطباعة" : "Sent to print", row.id)}><Printer size={12} /></button><button className="receipt-refund" onClick={() => action(ar ? "تم فتح الاسترجاع" : "Refund opened", row.id)}><RefreshCw size={12} /></button><button className="receipt-delete" onClick={() => { setRows(current => current.filter(item => item.id !== row.id)); action(ar ? "تم حذف" : "Deleted", row.id); }}><Trash2 size={12} /></button></div></td></tr>)}</tbody></table></div></section></div>;
}

function PointOfSalePage({ locale }: { locale: Locale }) {
  const ar = locale === "ar";
  const [showReceipts, setShowReceipts] = useState(false); const [operations, setOperations] = useState([
    { id: "ORD-869832", time: "4:39 م", amount: "29,500", status: "مسترجع", tone: "pink" },
    { id: "ORD-658779", time: "4:31 م", amount: "4,500", status: "مسترجع", tone: "pink" },
    { id: "ORD-267249", time: "4:31 م", amount: "10,000", status: "مسترجع", tone: "pink" },
    { id: "POS-NHI87H0Q2", time: "12:05 م", amount: "56,000", status: "مكتمل", tone: "green" },
    { id: "POS-L19W40ZRM", time: "11:35 ص", amount: "5,500", status: "مكتمل", tone: "green" },
    { id: "POS-EIF086CW4", time: "01:34 م", amount: "5,500", status: "مكتمل", tone: "green" },
  ]);
  const removeOperation = (id: string) => { setOperations(rows => rows.filter(row => row.id !== id)); toast.success(ar ? "تم تحديث سجل العمليات" : "Operations updated"); }; if (showReceipts) return <ReceiptHistoryPage locale={locale} onBack={() => setShowReceipts(false)} />;
  const salesTotal = operations.reduce((sum, row) => sum + Number(row.amount.replaceAll(",", "")), 0).toLocaleString("en-US");
  return <div className="pos-page">
    <div className="pos-page-head"><div><h1>{ar ? "نقطة البيع" : "Point of sale"}</h1><p>{ar ? "إدارة عمليات البيع اليومية" : "Manage daily sales operations"}</p></div><button className="pos-exit" onClick={() => toast.info(ar ? "تم تسجيل الخروج من نقطة البيع" : "Exited point of sale")}><LogOut size={12} />{ar ? "الخروج للوحة الإدارة" : "Back to admin"}</button></div>
    <div className="pos-summary-grid">
      <button className="pos-summary-card blue" onClick={() => toast.info(ar ? "فتح شاشة تسجيل الدخول لنقطة البيع" : "Opening POS sign-in")}><span className="pos-summary-icon"><Monitor size={19} /></span><div><strong>{ar ? "تسجيل الدخول لنقطة البيع" : "Sign in to POS"}</strong><small>{ar ? "الدخول لشاشة الفواتير وبدء البيع" : "Open invoices and start selling"}</small></div></button>
      <button className="pos-summary-card purple" onClick={() => setShowReceipts(true)}><span className="pos-summary-icon"><RefreshCw size={19} /></span><div><strong>{ar ? "سجل الإيصالات (تعديل/طباعة)" : "Receipt history (edit/print)"}</strong><small>{ar ? "سجل المبيعات المرتجعة وطباعة التقارير" : "Sales, returns and printable reports"}</small></div></button>
      <button className="pos-summary-card green" onClick={() => toast.success(ar ? `إجمالي مبيعات اليوم ${salesTotal} ريال يمني` : `Today's sales total: ${salesTotal} YER`)}><span className="pos-summary-icon"><BarChart3 size={19} /></span><div><strong>{ar ? "مبيعات اليوم" : "Today's sales"}</strong><b>{salesTotal} {ar ? "ريال يمني" : "YER"}</b><small>{operations.length} {ar ? "فاتورة" : "invoices"}</small></div></button>
    </div>
    <section className="panel pos-operations-card"><div className="pos-section-title"><h2>{ar ? "آخر العمليات" : "Recent operations"}</h2><button onClick={() => toast.success(ar ? "تم تحديث العمليات" : "Operations refreshed")}><RefreshCw size={13} />{ar ? "تحديث" : "Refresh"}</button></div><div className="pos-table-wrap"><table className="pos-table"><thead><tr><th>{ar ? "رقم الإيصال" : "Receipt number"}</th><th>{ar ? "الوقت" : "Time"}</th><th>{ar ? "المبلغ" : "Amount"}</th><th>{ar ? "الحالة" : "Status"}</th><th>{ar ? "إجراء" : "Action"}</th></tr></thead><tbody>{operations.map(row => <tr key={row.id}><td><b>{row.id}</b></td><td>{row.time}</td><td><strong>{row.amount} <small>{ar ? "ريال يمني" : "YER"}</small></strong></td><td><span className={`pos-status ${row.tone}`}>{row.status}<i /></span></td><td><button className="pos-row-action" onClick={() => removeOperation(row.id)}><Trash2 size={12} /></button></td></tr>)}</tbody></table></div></section>
  </div>;
}
