...
function PointOfSalePage({ locale }: { locale: Locale }) {
  ...
  const removeOperation = (id: string) => { setOperations(rows => rows.filter(row => row.id !== id)); toast.success(ar ? "تم تحديث سجل العمليات" : "Operations updated"); };
  return <div className="pos-page">
    ...
    <div className="pos-summary-grid">
      ...
      <button className="pos-summary-card purple" onClick={() => toast.info(ar ? "فتح سجل الإيصالات" : "Opening receipt history")}><span className="pos-summary-icon"><RefreshCw size={19} /></span><div><strong>{ar ? "سجل الإيصالات (تعديل/طباعة)" : "Receipt history (edit/print)"}</strong><small>{ar ? "سجل المبيعات المرتجعة وطباعة التقارير" : "Sales, returns and printable reports"}</small></div></button>
      <button className="pos-summary-card green" onClick={() => toast.success(ar ? "تم تحديث مبيعات اليوم" : "Today's sales refreshed")}><span className="pos-summary-icon"><BarChart3 size={19} /></span><div><strong>{ar ? "مبيعات اليوم" : "Today's sales"}</strong><b>117,000 {ar ? "ريال يمني" : "YER"}</b><small>12 {ar ? "فاتورة" : "invoices"}</small></div></button>
    </div>
